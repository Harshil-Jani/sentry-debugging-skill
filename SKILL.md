...
Token is typically provided via environment variable `SENTRY_AUTH_TOKEN` or directly by user.
...
curl -s "https://sentry.io/api/0/projects/{org_slug}/{project_slug}/issues/?query=is:unresolved&statsPeriod=24h" \
  -H "Authorization: Bearer $SENTRY_AUTH_TOKEN" | jq '.[] | {id, title, count, userCount, firstSeen, lastSeen}'
```
